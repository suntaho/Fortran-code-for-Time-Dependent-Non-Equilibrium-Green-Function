...
!    U(a,b,x) with integer ( kind = 4 ) b ( b = ?,?,... )
!
!  Licensing:
!
!    This routine is copyrighted by Shanjie Zhang and Jianming Jin.  However, 
...
!
!    Input, integer ( kind = 4 ) KD, the case code.
!    1, for cem(x,q)  ( m = 0,2,4,תת?)
!    2, for cem(x,q)  ( m = 1,3,5,תת?)
!    3, for sem(x,q)  ( m = 1,3,5,תת?)
!    4, for sem(x,q)  ( m = 2,4,6,תת?)
!
...
!
  ...
  else if ( a0 <= 20.0D+00 .or. &
	( x < 0.0D+00 .and. a0 < 30.0D+00 ) ) then
	ce1 = cmplx ( 1.0D+00, 0.0D+00, kind = 8 )
	cr = cmplx ( 1.0D+00, 0.0D+00, kind = 8 )
	do k = 1, 50000
	  cr = - cr * k * z / ( k + 1.0D+00 )**2
	  ce1 = ce1 + cr
	  if ( abs ( cr ) <= abs ( ce1 ) * 2.0D-16 ) then
		exit
	  end if
	end do
